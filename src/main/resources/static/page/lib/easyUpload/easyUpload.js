      ...
      var easyManager = {
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            $html += option.multi ? '<i class="easyUploadIcon noselect queue_check queue_check_allow_'+ file.allow +'" data-checked="'+(option.selected? 'yes':'no')+'" data-up="1">'+(option.selected? '&#xe61e;':'&#xe693;')+'</i>' : '';
            $html += '</div>';
            if (option.multi) {
              $(queueUl).append($html);
            } else {
