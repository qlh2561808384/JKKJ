...
(function (root, factory) {
	...
	Ssi_upload.prototype.initParameter = function(){
		this.uploadList = [];
        this.totalProgress = [];
        this.toUpload = [];
        this.imgNames = [];
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        $clearBtn.click(function () { //choose files completed and pending files
            thisS.clear();
            if(thisS.options.onClear){
                thisS.options.onClear();
            }
			thisS.initParameter();
        });
        ...
        $uploadBox.on('click', '.ssi-removeBtn', function (e) { //remove the file from list
			...
			if(!$currentTarget.hasClass('ssi-isupdate')){
				thisS.pending--; //reduce pending number by 1
			}else{
				thisS.totalFilesLength--;
				if(typeof thisS.options.onDeleteOne === 'function'){
					thisS.options.onDeleteOne(index);
				}
			}
...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        if (this.pending > 0) {
			...
			this.$element.find('.ssi-removeBtn:not(.ssi-isupdate)')
    ...
    var setElementMessage = function (thisS, index, msgType, msg, title) {
		...
		//新增 - Barlow Kane
		element.before(getResultMessage(msgType, msg, title, className))
		element.removeClass('ssi-abortUpload')
             .addClass('ssi-removeBtn ssi-isupdate')
             .children('span').addClass('trash10 trash')
             .removeClass('ban7w ban7');
		//end 新增
    ...
    var locale = {
        en: {
            ...
            sizeError: '$1 file size limit exceeded $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 file types are not supported',//$1=file extension ie(exe files are not supported)
            someErrorsOccurred: 'Error occurred!'
        },
        gr: {
            success: 'Επιτυχία',
